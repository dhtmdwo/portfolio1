...
import com.example.orderservice.menu_management.menu.model.MenuCount;
import com.example.orderservice.menu_management.menu.model.dto.TopMenuDto;
...
public interface MenuCountRepository extends JpaRepository<MenuCount, Long> {
    ...
    @Query("""
        SELECT new com.example.orderservice.menu_management.menu.model.dto.TopMenuDto(mc.storeId, mc.menu.id, SUM(mc.count))
        FROM MenuCount mc
        WHERE mc.usedDate BETWEEN :start AND :end
        GROUP BY mc.storeId, mc.menu.id
        ORDER BY mc.storeId, SUM(mc.count) DESC
        """)
